...
function startTagHandler(self, name, attr) {
...
function parse() {
  var self = this;
  var xml = jsEnv.isNode ? new expat.Parser('utf-8') : sax.createStream(true);
  this._parser = xml;
  this._suspended = false;
  var stack = [];
  var trace = {};
  ...
  if(jsEnv.isNode) {
    xml.on('startElement', function (name, attr) {
      startTagHandler(self, name, attr)
    });
  }
  if(jsEnv.isBrowser) {
  ...
  var endElementTagName;
  if (jsEnv.isNode) {
    endElementTagName = 'endElement';
  } else {
    endElementTagName = 'closetag';
  }
  xml.on(endElementTagName, function(name) {
  ...
  this._stream.on('end', function() {
    if (jsEnv.isNode && !xml.parse('', true)) {
      self.emit('error', new Error(xml.getError()+" in line "+xml.getCurrentLineNumber()));
    }
    self.emit('end');
  });
  if (jsEnv.isBrowser) {
    this._stream.pipe(xml);
  }
}
